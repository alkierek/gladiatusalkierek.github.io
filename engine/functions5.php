...
function updateUserItemSS1($userID, $old, $new)
{
	...
	$result = mysql_query("SELECT * FROM ". ((substr($old, 0, 4) == "p513")? "inventory2" : "sshop1") ." WHERE id = '". $userID ."'") or die(mysql_error());
	$result3 = mysql_query("SELECT * FROM ". ((substr($old, 0, 4) == "p513")? "doll" : "sshop1") ." WHERE id = '". $userID ."'") or die(mysql_error());
	if (mysql_num_rows($result) > 0)
	{
		...
		if ($array[$old] != null)
		{
			...
			else if (substr($old, 0, 4) != "p513" && substr($new, 0, 4) == "p513")
			{
				if (substr($old, 0, 4) == "p274") {
					...
					mysql_query("UPDATE inventory2 SET ". $new ." = '". $array3[$old] ."' WHERE id = '". $userID ."'");
					mysql_query("UPDATE doll SET ". $old ." = NULL WHERE id = '". $userID ."'");
				}
			}
			else if (substr($old, 0, 4) == "p513" && substr($new, 0, 4) != "p513")
			{
				if (substr($new, 0, 4) == "p274") {

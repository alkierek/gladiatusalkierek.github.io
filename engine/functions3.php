...
function itemDescriptionIdentifier($key, $value)
{
	...
	switch ($key)
	{
		...
		case "soul-bound":
			if ($value == true)
				$html = "<tr><td style=\'color:#DDD; font-weight: bold; font-size:8pt\' colspan=\'2\' nowrap=\'nowrap\'>(Soul bound)</td></tr>";
		break;
		case "armour":
...
function addItemsToInventory($playerID)
{
	...
	// MAKE A QUERY
	$result = mysql_query("SELECT * FROM inventory2 WHERE id=". $playerID ."");
	...
	for ($y = 1; $y <= 8; $y++)
	{
		for ($x = 1; $x <= 5; $x++, $id++)
		{
			...
			$script .= "AddCharDiv(\"p513_". $y ."_". $x ."\", ". (($y - 1) * 32) .", ". (($x - 1) * 32) .", ". $type .", ". $size .", ". $bsize .", 16777215, ". $iid .", ". $price .", ". $rubies .", ". $premium .", 0, 0, ". $bound .", ". $level .", 0, \"". $item_description_html_layout ."\", [ ]);\n";
		}
	}
	mysql_close();
...
function addItemImagesToInventory($playerID)
{
	...
	for ($y = 1; $y <= 8; $y++)
	{
		for ($x = 1; $x <= 5; $x++, $id++, $zIndex++)
		{
			...
			$html .= "<div id=\"p513_". $y ."_". $x ."\" style=\"position: absolute; width: ". (($y == 1 && $x == 1)? 64 : 32) ."px; height: ". (($y == 1 && $x == 1)? 64 : 32) ."px; top: ". (($y - 1) * 32) ."px; left: ". (($y - 1) * 32) ."px; z-index: ". $zIndex ."; cursor: move; visibility: visible;\" onmouseover=\"return escape(dd.elements.p513_". $y ."_". $x .".tooltip)\">". $image ."</div>\n";
		}
	}
	mysql_close();
...
function updateUserItem($userID, $old, $new)
{
	...
	$result = mysql_query("SELECT * FROM ". ((substr($old, 0, 4) == "p513")? "inventory2" : "doll") ." WHERE id = '". $userID ."'") or die(mysql_error());
	$result2 = mysql_query("SELECT * FROM ". ((substr($new, 0, 4) == "p513")? "inventory2" : "doll") ." WHERE id = '". $userID ."'") or die(mysql_error());
	if (mysql_num_rows($result) > 0)
	{
		...
		if ($array[$old] != null && $array2[$new] == null)
		{
			...
			if (substr($old, 0, 4) == "p513" && substr($new, 0, 4) == "p513")
			{
				mysql_query("UPDATE inventory2 SET ". $old ." = NULL, ". $new ." = '". $array[$old] ."' WHERE id = '". $userID ."'");
			}
			else if (substr($old, 0, 4) != "p513" && substr($new, 0, 4) == "p513")
			{
				mysql_query("UPDATE inventory2 SET ". $new ." = '". $array[$old] ."' WHERE id = '". $userID ."'");
				mysql_query("UPDATE doll SET ". $old ." = NULL WHERE id = '". $userID ."'");
			}
			else if (substr($old, 0, 4) == "p513" && substr($new, 0, 4) != "p513")
			{
				mysql_query("UPDATE doll SET ". $new ." = '". $array[$old] ."' WHERE id = '". $userID ."'");
				mysql_query("UPDATE inventory2 SET ". $old ." = NULL WHERE id = '". $userID ."'");
			}
			else
			{
...
function dropPackageToInvetory($userID, $pid, $new)
{
	...
	$result2 = mysql_query("SELECT * FROM inventory2 WHERE id = '". $userID ."'") or die(mysql_error());
	if (mysql_num_rows($result) > 0)
	{
		$array = mysql_fetch_array($result); 
		$array2 = mysql_fetch_array($result2);
		if ($array2[$new] == null)
		{
			...
			mysql_query("UPDATE inventory2 SET ". $new ." = " . $piid . " WHERE id = '". $userID ."'");

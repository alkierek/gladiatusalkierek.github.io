			...
			if (typeof (t_tj = tags[j]).onmouseover == 'function') {
				...
				if (t_tj.onmouseover.toString().indexOf(esc) != -1)
				{
					...
					if (!tt_n6 || tt_n6 && (over = t_tj.getAttribute('onmouseover')) && over.indexOf(esc) != -1)
					{
                        ...
                        if (Browser.Platform.ios || Browser.Platform.android) t_tj.onclick = t_tj.onmouseover;
...
//			(tt_n4? 21 : 0);
        ...
        if (document.getElementById('container_game') === null) {
            rect = document.getElementById('container_start').getBoundingClientRect();
        } else {
            rect = document.getElementById('container_game').getBoundingClientRect();
        }

        xlim = tt_Int(rect.right - 50) +
			tt_Int(0) -
			tt_objW -
			(tt_n4? 21 : 0);
